...
- owner-accepted Lupus Noctis production illustrations were generated by the project owner through the built-in OpenAI/Codex image-generation workflow and then manually selected; the exact current inventory belongs to `artwork-pack.json`, while provenance details and unresolved release-rights verification belong to `docs/THIRD-PARTY.md`, `LUPUS-NOCTIS.md`, and its linked records;
