    ...
    public InterpretationSourceCompilationResult Load(string sourceRoot)
    {
        ...
        foreach (var file in files.Where(static item => item.Relative != "interpretation-pack.json"))
        {
            ...
            if (kind == "synthesis" && segments.Length == 5 && segments[4].EndsWith(".json", StringComparison.Ordinal))
            {
                var value = Read(
                    file,
                    static bytes => TarotInterpretationJson.Parse<TarotSynthesisResourceDocument>(bytes),
                    TarotInterpretationJson.Serialize,
                    TarotInterpretationValidator.ValidateSynthesisResource,
                    bag);
                if (value is null) { invalidBundles++; continue; }
                var type = TarotSchemaText.Get(value.ResourceType, TarotSchemaText.SynthesisResourceTypes);
                if (type != segments[3] || value.ResourceId.Value != segments[4][..^5]) { bag.Error("source.identity-path", file.Relative, "Synthesis identity does not match path."); noncanonical++; invalidBundles++; continue; }
                if (!identities.Add($"y|{locale.Value}|{type}|{value.ResourceId.Value}")) { bag.Error("source.duplicate", file.Relative, "Duplicate synthesis identity."); duplicates++; invalidBundles++; continue; }
                synthesis[locale].Add(value); validBundles++; continue;
            }
        ...
        var counts = new Dictionary<string,int>(StringComparer.Ordinal)
        {
            ...
            ["synthesisResources"] = synthesis.Sum(static pair => pair.Value.Count),
            ["missingSynthesisResources"] = manifest.DeclaredLocales.Sum(locale => TarotThreeCardSynthesisContract.RequiredResources.Count - synthesis[locale].Count)
        };
