...
- A20 is owner-accepted and promoted byte-for-byte: `major.hierophant`, `minor.cups.nine`, and `minor.swords.queen` bring Lupus Noctis to 54/78 production illustrations with fallback for 24 cards; all failed attempts and literal owner-rejection defects remain in their records. A21 remains outside the manifest, production cards, and Release output with exactly three Pending candidates: `major.world` passed at `G1` with one physically rigged hoop; `minor.wands.six` used `G3` critical recovery and now contains exactly `3 + 3 = 6` complete staffs, four visible hooves, and both boots in visible stirrups; `minor.cups.two` passed at `G1` with exactly two people, two cups, and no other container. All three pass hard-contract and independent absurdity gates; no owner acceptance or promotion is implied.
...
Continue ART-LN with owner visual review of the three Pending A21 candidates. Technical review found no remaining blocker, but only the owner may accept or reject them. A21-P, A22, and AP1 do not start automatically.
...
- Owner visual review of A21 is the current ART-LN gate. No technical blocker remains in the three Pending candidates, but owner acceptance is external. A21-P, A22, AP1–AP5, and PKG1 remain unstarted.
