...
- Tarot semantic decks, artwork packs, presentation skins, back variants, and interpretation packs are independent. `classic` is the active identity and sole built-in source; every skeleton module remains `ready = false`. The pure resolver applies requested locale then English then Russian then typed silent absence, stops on a broken ready locale, validates manifest/index/content hashes, loads one canonical entry lazily, and uses bounded caches. `two-cards` owns 3003 unordered identities/12,012 authored states; `three-cards` composes 468 position entries and three pair relations deterministically. Reveal state is Presentation-owned, and no interpretation prose or corpus is implemented.
