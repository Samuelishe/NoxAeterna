...
- A22 is owner-accepted and promoted byte-for-byte: `minor.pentacles.queen`, `minor.swords.king`, and `minor.wands.ace` bring Lupus Noctis to 60/78 production illustrations with fallback for 18 cards. The reusable artwork skill now applies a target-card-size significance/count-confusion gate; the Queen record preserves G1–G3 while noting that its tiny-fastener and small-earring corrections were overly strict. A23 remains outside the manifest, production cards, and Release output with exactly three Pending candidates: `minor.wands.king` passed at `G1`, `minor.swords.eight` used `G2` to replace a genuinely tight wrist binding with a visibly escapable loop while retaining exactly `4 + 4 = 8` complete swords, and `minor.pentacles.four` passed at `G1` with exactly `2 + 2 = 4` door seals. All three pass hard-contract and independent absurdity gates; no owner acceptance or production promotion is implied.
...
Continue ART-LN with owner visual review of the three Pending A23 candidates. Technical review found no remaining blocker, but only the owner may accept or reject them. A23-P, A24, and AP1 do not start automatically.
...
- Owner visual review of A23 is the current ART-LN gate. No technical blocker remains in the three Pending candidates, but owner acceptance is external. A23-P, A24, AP1–AP5, and PKG1 remain unstarted.
