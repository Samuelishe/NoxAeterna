...
- A21 is owner-accepted and promoted byte-for-byte: `major.world`, `minor.wands.six`, and `minor.cups.two` bring Lupus Noctis to 57/78 production illustrations with fallback for 21 cards; every generation and owner decision remains in its record. A22 remains outside the manifest, production cards, and Release output with exactly three Pending candidates. `minor.pentacles.queen` used `G3` to remove extra circular metal and now contains one Queen, one ewe, one newborn lamb and one complete pentacle; `minor.swords.king` used `G2` to make hand-to-door contact literal while retaining one complete sword and a causal counterweight fraud; `minor.wands.ace` used `G3` to remove extra lights, align wind and restore both full feet while retaining one complete signal wand and one flame. All three pass hard-contract and independent absurdity gates; no owner acceptance or production promotion is implied.
...
Continue ART-LN with owner visual review of the three Pending A22 candidates. Technical review found no remaining blocker, but only the owner may accept or reject them. A22-P, A23, and AP1 do not start automatically.
...
- Owner visual review of A22 is the current ART-LN gate. No technical blocker remains in the three Pending candidates, but owner acceptance is external. A22-P, A23, AP1–AP5, and PKG1 remain unstarted.
