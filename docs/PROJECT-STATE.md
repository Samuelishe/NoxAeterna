...
- A11 is visually accepted by the owner: `major.strength`, `minor.cups.four`, and `minor.pentacles.king` are promoted into the Lupus Noctis partial pack, which now contains 27/78 production illustrations. A12 generation is incomplete because the permitted `minor.cups.ten` correction still violated its exact cup count; the two valid A12 candidates remain outside production.
...
Complete the blocked `minor.cups.ten` generation only after explicit owner direction on whether to exceed the one-correction limit, then continue ART-LN with owner visual review of all three A12 candidates. The post-A12 batch remains unselected, and AP1 does not start automatically.
...
- Lupus Noctis A0 and A1 remain rejected. The owner accepted all A2, A4, A5, A6, A7, A8, A9, A10, and A11 cards; the built-in partial pack now owns 27/78 production illustrations and uses the existing programmatic face for omitted cards. A12 remains outside the manifest and is incomplete until a contract-valid `minor.cups.ten` candidate exists. Detailed card state, prompts, provenance, hashes, and casting policy belong only to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md`.
...
- Explicit owner direction on the exhausted `minor.cups.ten` correction allowance is the current ART-LN gate; the post-A12 batch remains deliberately unselected. AP1–AP5 and PKG1 are staged work, not blockers for ART-LN.
