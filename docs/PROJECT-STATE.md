...
- A16 is owner-accepted and promoted: `major.magician`, `minor.wands.knight`, and `minor.cups.page` bring Lupus Noctis to 42/78 production illustrations. Their owner notes remain explicit: Magician's accepted one-card pop-fantasy association must not become a template, Knight's tail-margin note remains non-blocking, and Page's oversized rail wolf head must not become a recurring stamp. Durable rules now treat owner examples as idea-space, require meaning-derived mood fidelity, use a wolf-prominence spectrum, preserve grounded supernatural and mature-content boundaries, and review dense pop-fantasy resemblance. The three A17 candidates remain Pending owner review.
...
Continue ART-LN with owner visual review of the three Pending A17 candidates. The post-A17 batch remains unselected, and A17-P, A18, and AP1 do not start automatically.
...
- Lupus Noctis A0 and A1 remain rejected. The owner accepted all A2, A4, A5, A6, A7, A8, A9, A10, A11, the two promoted A12 cards, all four A13 cards, and all three A14, A15, and A16 cards; the built-in partial pack now owns 42/78 production illustrations and uses the existing programmatic face for omitted cards. A17 remains a bounded three-card Pending review batch outside the manifest and Release output. Detailed card state, prompts, provenance, hashes, casting policy, diversity gates, and accepted exceptions belong only to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md`.
...
- Owner visual review of A16 is the current ART-LN gate; the post-A16 batch remains deliberately unselected. AP1–AP5 and PKG1 are staged work, not blockers for ART-LN.
