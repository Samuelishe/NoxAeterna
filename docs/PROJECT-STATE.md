...
- A15 is owner-accepted and promoted: `major.high-priestess`, `minor.swords.seven`, and `minor.cups.ace` bring Lupus Noctis to 39/78 production illustrations. Their accepted exceptions remain explicit: the High Priestess background grille/face tangency, Seven of Swords' weak or absent wolf-world anchor, and Ace of Cups' low-novelty handoff staging. Durable wolf-anchor legibility, batch casting contrast, narrative novelty, and occlusion/tangency gates now govern later batches. The three A16 candidates remain Pending owner review.
...
Continue ART-LN with owner visual review of the three Pending A16 candidates. The post-A16 batch remains unselected, and AP1 does not start automatically.
...
- Lupus Noctis A0 and A1 remain rejected. The owner accepted all A2, A4, A5, A6, A7, A8, A9, A10, A11, the two promoted A12 cards, all four A13 cards, all three A14 cards, and all three A15 cards; the built-in partial pack now owns 39/78 production illustrations and uses the existing programmatic face for omitted cards. A16 remains a bounded three-card Pending review batch outside the manifest. Detailed card state, prompts, provenance, hashes, casting policy, diversity gates, and accepted exceptions belong only to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md`.
