...
- Tarot semantic decks, artwork packs, skins, backs, and interpretation packs are independent. `classic` remains the sole built-in pack; only `ru + single-card` is ready and the other seven locale/mode declarations remain unready. The resolver retains read-only SQLite stores, requested→English→Russian→silent absence, and broken-ready stop behavior. Five-section presentation, deterministic tags, metrics, reveal gating, and silent `NoContent` remain unchanged. RU oriented-pair source is intentionally partial at 500/3,003 bundles; EN, three-card-position, and synthesis prose remain absent.
