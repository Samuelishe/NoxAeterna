...
- A19 is owner-accepted and promoted byte-for-byte: corrected `major.wheel-of-fortune`, `minor.cups.king`, and `minor.pentacles.seven` bring Lupus Noctis to 51/78 production illustrations with fallback for 27 cards. A20 remains outside the manifest and Release output with three corrected Pending candidates ready for owner visual review: `major.hierophant` used `G2` to replace technical-demonstration role ambiguity with a formal guild initiation; `minor.cups.nine` used `G3` critical recovery to replace the blocking ten-cup regression with exactly `3 + 3 + 3 = 9`; and `minor.swords.queen` used `G2` to replace the unsupported outer-rail figure with a continuous usable checkpoint. All three passed hard-contract and independent absurdity gates; no owner acceptance or production promotion is implied.
...
Continue ART-LN with owner visual review of the three corrected Pending A20 candidates. Technical review found no remaining blocker, but only the owner may accept or reject them. A20-P, A21, and AP1 do not start automatically.
...
- Owner visual review of corrected A20 is the current ART-LN gate. No technical blocker remains in the three Pending candidates, but owner acceptance is external. A20-P, A21, AP1–AP5, and PKG1 remain unstarted.
