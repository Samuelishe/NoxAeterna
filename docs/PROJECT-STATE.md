...
- A12 is closed with two owner-accepted production cards: `major.chariot` and `minor.swords.two`. Lupus Noctis now contains 29/78 production illustrations; `minor.cups.ten` was explicitly carried into the four-card A13 batch, whose candidates remain Pending owner review.
...
Continue ART-LN with owner visual review of the four Pending A13 candidates. The post-A13 batch remains unselected, and AP1 does not start automatically.
...
- Lupus Noctis A0 and A1 remain rejected. The owner accepted all A2, A4, A5, A6, A7, A8, A9, A10, A11, and the two promoted A12 cards; the built-in partial pack now owns 29/78 production illustrations and uses the existing programmatic face for omitted cards. A13 remains a bounded four-card Pending review batch outside the manifest. Detailed card state, prompts, provenance, hashes, and casting policy belong only to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md`.
...
- Owner visual review of A13 is the current ART-LN gate; the post-A13 batch remains deliberately unselected. AP1–AP5 and PKG1 are staged work, not blockers for ART-LN.
