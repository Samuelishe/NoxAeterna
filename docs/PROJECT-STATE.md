...
- Tarot semantic decks, artwork packs, presentation skins, back variants, and interpretation packs are independent. `classic` is the active compile-time pack identity and future selectable default; readiness is manual per pack/locale/mode, fallback is requested locale then English then Russian then silent absence, and one resolved mode uses one locale. `two-cards` owns 3003 unordered identities/12,012 authored states; `three-cards` composes 468 position entries and three pair relations deterministically. Reveal state is Presentation-owned, and no interpretation prose or corpus is implemented.
