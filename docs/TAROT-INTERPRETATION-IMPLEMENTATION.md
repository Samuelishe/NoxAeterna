...
The playable workspace keeps a narrow App adapter over that resolver. `single-card` resolves after its assignment is revealed; `two-cards` resolves exactly one unordered oriented-pair entry only after both technical draw slots are revealed. `three-cards` progressively resolves revealed position entries plus only the revealed adjacent `past-present` and `present-future` relations through mode `three-cards`; it requests the exact planner-selected trajectory and transition synthesis resources through the same ready/fallback/store boundary only after all three positions and both relations resolve. Presentation converts resolved entries and trusted pack-local section, position, relation, and vocabulary labels into typed UI models, so Avalonia receives localized labels and authored valence/intensity rather than storage DTOs or raw concept IDs. Pair section chrome (`interaction` and `direction`) is selected with the resolved interpretation locale, including locale fallback.
