...
## T-UX1A/T-UX2A Reading Surface and Reveal Preferences

The fixed control panel sits above one stretching reading surface. For `single-card`, a render-independent Presentation contract selects a wide two-column composition only when the content width can hold the accepted `378` DIP card column, a `28` DIP gap, and a `400` DIP minimum interpretation column. The card remains in the bounded left column and fits available height between the existing `216` and `378` DIP width limits; the right column owns vertical interpretation scrolling and limits text to a `720` DIP readable measure. Below that derived threshold, single-card returns to the ordered card → tags → interpretation stack and its outer vertical scroller.
