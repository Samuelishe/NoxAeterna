...
| Microsoft.Data.Sqlite | Microsoft | MIT | ADO.NET provider for compiling and reading immutable `.noxinterp` SQLite interpretation packages | https://www.nuget.org/packages/Microsoft.Data.Sqlite/10.0.10 | Added in `NoxAeterna.Interpretation.Sqlite`, pinned to `10.0.10`; this is not the future mutable user-history store. |
| SQLitePCLRaw.lib.e_sqlite3 | Eric Sink / SQLitePCLRaw contributors | Apache-2.0 | Bundled native SQLite library used by Microsoft.Data.Sqlite | https://www.nuget.org/packages/SQLitePCLRaw.lib.e_sqlite3/2.1.12 | Directly pinned to `2.1.12` so the adapter does not resolve the vulnerable earlier transitive version. |
...
| Name | Author or organization | License | Purpose in project | Official repository or site | Notes |
| --- | --- | --- | --- | --- | --- |
| Lupus Noctis owner-accepted production illustrations | Nox Aeterna project owner; earlier waves used the built-in OpenAI/Codex image workflow, while later handoff waves were created by the owner together with ChatGPT outside Codex | Release and redistribution terms require separate owner/legal verification before distribution; no broader rights claim is made here | Curated production illustrations in the built-in complete 78-card Tarot artwork pack | N/A — project-owned generated material | Every production image was manually accepted by the owner. Codex performed technical import only for the outside-Codex handoff waves. No third-party images, rejected generations, or superseded handoff candidates are shipped. The exact current inventory belongs to `artwork-pack.json`; available provenance, review notes, hashes, and production paths belong to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md` and its linked records. |
