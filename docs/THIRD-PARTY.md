...
| Name | Author or organization | License | Purpose in project | Official repository or site | Notes |
| --- | --- | --- | --- | --- | --- |
| Lupus Noctis owner-accepted production illustrations | Nox Aeterna project owner, using the built-in OpenAI/Codex image-generation workflow | Release and redistribution terms require separate owner/legal verification before distribution; no broader rights claim is made here | Curated production illustrations in the built-in partial Tarot artwork pack | N/A — project-owned generated material | Generated from project-authored prompts and manually selected by the owner. No third-party images are shipped. Rejected generations are not distributed. The exact current inventory belongs to `artwork-pack.json`; full prompts, review notes, hashes, production paths, and generation provenance belong to `resources/assets/tarot/artwork-packs/lupus-noctis/LUPUS-NOCTIS.md` and its linked records. |
