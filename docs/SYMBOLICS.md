...
For Tarot tags, a language-neutral concept ID identifies semantic meaning; a visible label belongs to the selected interpretation pack and locale. Packs may reuse an ID for genuinely equivalent meaning or define package-specific concepts, so labels alone do not establish semantic identity. The Tarot meaning and quality contract belongs to [`TAROT-INTERPRETATION-CONTENT.md`](TAROT-INTERPRETATION-CONTENT.md); the exact `conceptId` vocabulary/tag serialization belongs to [`TAROT-INTERPRETATION-IMPLEMENTATION.md`](TAROT-INTERPRETATION-IMPLEMENTATION.md), not to Symbolics or user-facing prose here.

Oriented pair states, temporal relation IDs, reversal mechanisms, and trajectory classifications are interpretation-pack composition metadata rather than new global Symbolics relationships. Their canonical IDs, corpora, and routing belong to [`TAROT-INTERPRETATION-MODES.md`](TAROT-INTERPRETATION-MODES.md).
